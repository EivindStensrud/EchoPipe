...
# ---------------------------------------------------------------------------
# This scripts mines potential reference sequences for eDNA metabarcoding studies using sequence similarity.
...
from Bio.Blast.Applications import NcbiblastnCommandline # So that we can BLAST locally on the computer. 
...
from tqdm import tqdm # Used for progressbar
import re
...
def update_last_line(file_path, progress_info, idx):
    ...
    # Use the `idx` parameter to control line overwriting/appending
...
def error_occurrence(error, species, retry_delay):
    print(f"An error occurred: {error}") # Let's us know an error has occurred. Not as specific as the HTTP one above, but this covers for less common errors as well.
    ...
    return error, species # This return statement allows "error" to be saved from the function. saved_variable = error_occurrence(error) means that "e" is saved in saved_variable.
...
#Update the counter file with the new counter value for the current date.
def update_counter(counter_file, date, counter):
    try:
        ...
        print(f"Error updating the counter file: {e}")

def get_log_file_name(logs_dir):
    """Generate a unique log file name based on the current date and counter."""
    ...
    # Read current counter for the date
    counter = read_counter(counter_file, DATE)
        
    # Formulate the run name
    run_name = f"{DATE}_{counter}"
    log_file_path = os.path.join(logs_dir, f"{run_name}_log.txt")
    log_file_path = os.path.abspath(log_file_path)
    # Update the counter file with the new value
    update_counter(counter_file, DATE, counter)
...
# Define how many threads to use to parallelize the scripts. Default = 7
# From experience, 7 is a good trade of between speed and stabile connection.
def number_threads():
        ...
        for i in range(max_retries):
            try:
                ...
                if taxid_record["Count"] == str(0) or "ErrorList" in taxid_record:  # When this is True it means that the search (i.e. from a misspelled name or a non-existing one) came out as negative and no data was retrieved.
                    return {'species_not_found': species} # The species' name is added to the species_not_found list.
                    ...
                    return {'duplicate_species': species} # If it does exist, it means it is a duplicate entry and is thus added to the duplicate_species list.
                    ...
                    return {'filtered_species_collection': taxid_entry, 'species_found': species} # taxid_entry is appended to the list filtered_species_collection.
            ...
            except HTTPError as error:  # Specific for HTTP errors.
                last_exception = error_occurrence(error, species, retry_delay)
                return {'failed_information_downloads': f"HTTPError: Failed to download information about {species} - {error}"}

            except Exception as e: # Non-specified errors.
                last_exception = error_occurrence(e, species, retry_delay)
                return {'failed_information_downloads': f"Exception: Failed to download information about {species} - {e}"}
...
def process_species_accession_number(specie, search_settings, max_search, batch_size, max_retries, sort_by_sequence_length, consecutive_fail_counter, log_file, failed_accession_downloads, type_of_download, run_name, create_directory, repeated_failures, error_occurrence, over_cap_csv_list):
            species_directory = f"Species/{specie}" # Species' directory location from the working directory, e.g., Species/Alosa_alabamae
            ...
            search = str(specie.replace("_", " ") + search_settings) # Defining the search term we want to use with esearch. E.g. Homo sapiens[Organism]. We also have implemented a sequence length (SLEN) limit to avoid crashes by ignoring massive sequences.
            ...
            for i in range(max_retries):
                ...
                except Exception as error: # Non-specified errors.
                    last_exception = error_occurrence(error, specie, retry_delay)
            ...
            with open(accession_file_1, "w") as file: # File where the accession numbers from run_name are stored.
                for accession_batch in accession_collection:
                    for accession_number in accession_batch:
                        file.write(accession_number + "\n")

            log_progress = f"Most recent species: {specie.replace('_', ' ')}."
            return specie
...
def download_fasta_sequences(specie, run_name, taxid_collection, sequence_batch_size, max_retries, log_file, consecutive_fail_counter, failed_information_downloads, type_of_download):
            ...
            # Step 1: Retrieve header lineage from the taxid collection
            header_lineage = None
            with open(taxid_collection, "r") as file: # Opens taxid_collection...
                ...
                for line in species_info: # Loops through each line
                    if specie in line: # Checks to see if species (e.g. Triturus_cristatus) exists inside of each line as it loops through.
            ...
            # Step 2: Retrieve accession numbers for the species
            path_to_new_acc = f"Species/{specie}/{specie}_new_accession_numbers_{run_name}.txt"  # Path to the file with the new accession numbers, whose sequences are to be retrieved.
            try:
                with open(path_to_new_acc, "r") as file:
                    new_accession_numbers = [line.strip() for line in file.readlines()] # The species' (new) accession numbers are stored in new_accession_numbers. # Removes "\n" from each line.
            ...
            # Handle repeated failures logic before proceeding
            repeated_failures(consecutive_fail_counter, log_file, failed_information_downloads, type_of_download)

            # Step 3: I attempts to download sequences using batching approach
            for i in range(max_retries):
                try:
                    with open(f"Species/{specie}/{specie}_new_seqs_{run_name}.fasta", "w") as file: # Creates and opens a file where the sequences from the (new) accession numbers are to be written into.
                        # Ensure batching size is appropriate
                        ...
                        # Download sequences in batches using Entrez
                        for i in range(0, len(new_accession_numbers), sequence_batch_size):
                            ...
                            with Entrez.efetch(db="nucleotide", idtype="acc", id=batch, rettype="fasta", retmode="text") as handle: # Performs an efetch to get the sequences.
                            ...
                            time.sleep(1)  # Brief pause for each sequence batch
                    #print(f"New sequence(s) for \033[32m{species.replace('_', ' ')}\033[0m has been downloaded.") # Not in use, makes the output more messy.  # Prints a message that new sequence(s) have been downloaded. Optional.
                    consecutive_fail_counter = 0 # Makes sure the timer is set to 0 after successful iterations.
                    break
                ...
                #print(f"Unable to download sequence(s) for {specie}.")
            ...
            # Step 4: Update the log file with progress
            log_progress = f"Most recent species: {specie.replace('_', ' ')}."
        

# Conduct Blast allignment to find the longest alignment of potential reference sequences.
def perform_blast_operations(
        new_accession_species, run_name, database_path, analysed, min_length,
        ...
        if reuse_sequences: # If previous sequence files are to be run, file_to_blast is made into a list in order to be able to hold multiple file names.
            # Collect and concatenate files for blasting
            blast_again = []
            ...
            file_to_blast = f"{species_directory}/{specie}_new_seqs_{run_name}.fasta" # We want to BLAST the current run_name's sequence(s).
            finished_accession_numbers = f"{species_directory}/{specie}_{run_name}_accession_numbers.txt"

        blast_output = f"{species_directory}/{specie}_{run_name}_fasta_blast_results.txt" # The name of the BLAST output.
        ...
        if reuse_sequences or os.path.exists(file_to_blast): 
            blastn_cline = NcbiblastnCommandline(
                ...
                max_target_seqs=100, # The limit to our output. When set to 100, we get the 100 best hits.
                evalue=evalue,
                word_size=11,
                reward=2,
                penalty=-3,
            ...
            if stderr: # If there is no error, then stderr is empty and no error message is printed.
                #print(f"Error: {stderr}") #Not printed, makes the output look weird.
                ...
                analysis_completed(finished_accession_numbers, analysed_accession_numbers, reuse_sequences)  # Accession numbers are added to {species}_analysed_accession_numbers.txt once BLAST is done.
                nothing_new_to_blast.append(specie.replace('_', ' '))
                #print(f"BLAST completed for {specie.replace('_', ' ')}. However, without any new results.") #Not printed, makes the output look weird.
                ...
                analysis_completed(finished_accession_numbers, analysed_accession_numbers, reuse_sequences) # Accession numbers are added to {species}_analysed_accession_numbers.txt once BLAST is done (just like in the elif-statement).
                #print(f"BLAST completed for {specie.replace('_', ' ')}.") #Not printed, makes the output look weird.
                
        else:
            print(f"\033[31mNo blasting for {specie.replace('_', ' ')}: {file_to_blast} not found. \033[0m") # Occurs if file_to_blast does not exist (which is the case if no new sequence(s) was found). Might replace this.
...
def main():
    ...
    group_1.add_argument('-E', '--evalue', type=int, default=10,
        help="The E-value used for BLAST. The higher the value entered, the more stringent the BLAST becomes. Default = 12, meaning: 10^-12.")
    group_1.add_argument('-R', '--repeat', action='store_true', default="",
        help="No new sequences are downloaded. The curation step needs to be ran first. Instead, all previously downloaded sequences are BLASTed against the input database. Note: this options resets the file containing analysed accession numbers and the output will be curated from the start.")
    ...
    # Update counter after finalizing the unique run_name
    update_counter(counter_file, DATE, i + 1 )
    
    #Sets log directory and file.
    ...
    search_settings = '[Organism] AND biomol_genomic[PROP]' + max_length_search + mitochondria_on + ribosomal_on + custom_query # Baseline and complimentary search terms. Can, for example, filter away all non-annotated mitochondrial sequences.
    ...
    ###
    ### DOWNLOAD TAXONOMY FOR THE SPECIES IN SPECIES LIST
    ###
    ...
    if not use_old_taxid: # Skips this step if the user decided to go with the previous taxid collection - saves time proportional to the number of input species.
     ...
     # Open the file and read species list
    ...
    max_retries = 5  # Assuming `max_retries` is defined elsewhere in your original script

    loop_timer = time.time()

    # Parallelization of taxonomy download
    ...
    if species_not_found:
        ...
        write_lines_from_list(species_not_found, error_collection, log_file, f"Non-valid species name found. See {error_collection}\n")
    ...
    if duplicate_species:
        ...
        write_lines_from_list(duplicate_species, duplicate_collection, log_file, f"Duplicate species name found. See {duplicate_collection}\n")

    write_lines_from_list(filtered_species_collection, taxid_collection, log_file, f"The following species had sequences on NCBI. See {species_found}\n")
    ...
    ###
    ### FINDING NEW UNANALYZED ACCESSION NUMBERS
    ###
    ...
    print("\nRecommendation:\n")
    print("See tutorial for detailed explanation: https://github.com/EivindStensrud/EchoPipe/tree/main\n")
    print("To help curate the database run the following code:\n")
    print(f"Next code line:\n\033[32mpython Echopipe_database_curation.py BLAST_results/{run_name}_to_curate.fasta\033[0m \n")

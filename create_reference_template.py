...
parser = argparse.ArgumentParser(
    ...
    description="A script used to have a reference template database to be used for ECHoPipe.",
    epilog="Version 1.0")

group_1 = parser.add_argument_group("Arguments used to generate an uncurated reference template.\nThe required inputs are input_file, -f (--forward) and -r (--reverse)")
group_1.add_argument('input_file', type=str, nargs='?',
    help="Either a text file with a list of species names or a fasta file that is to be converted into the reference template database.")
group_1.add_argument('-f', '--forward',
    help="The forward primer that was or is to be used to generate eDNA sequences.")
group_1.add_argument('-r', '--reverse',
    help="The reverse primer that was or is to be used to generate eDNA sequences.")
group_1.add_argument('-e', '--email', type=str, default=None,
    help="Always tell NCBI who you are.")
group_1.add_argument('-a', '--api_key', type=str, default=None,
    help="The user's NCBI API key.")
group_1.add_argument('-q', '--query',
    help="The search result will include the user input search term(s). For example: if the user wish to include a term, make sure to put 'AND' followed by the search term. Likewise, in order to exclude a term write instead 'NOT' in front without ''.")
...
group_1.add_argument('-m', '--max', type=int, default=1,
    help="The amount of sequences that are downloaded per species that may be increased for greater coverage, but is recommended to always remain single digit. While depending on the computational power, it is suggested to not exceed a total of 500 sequences. Default = 1.")
group_1.add_argument('-p', '--provided_sequences', action='store_true', default="",
    help="This flag is used when the input file provided is a fasta file of their own to be turned into a reference template database, rather than downloading their own sequences.")

group_2 = parser.add_argument_group("The argument used to finalize and create the reference template database")
...
if not args.Complete:
    if not all ([args.input_file, args.forward, args.reverse]):
        parser.error("An input file with a list of species along with forward (-f, --forward) and reverse (-r, --reverse) primers must be provided when generating the reference template with exception for when -C, --Complete is used to finalize it.")
    else:
            ...
            with open(log_file, "w") as file:
                ...
                if not skip_download:
                    file.write(
                        ...
                        f'Search term: [Organism] AND ("20"[SLEN] : "22000"[SLEN]) AND biomol_genomic[PROP] NOT "unverified" {args.query}')
                ...
                with open(ncbi_output_file, "w") as file:
                    for idx, species in  tqdm(enumerate(species_list, start=1), total=len(species_list), desc="Going through the species list"):
                        ...
                        for _ in range(max_retries):
                            try:
                                search = str(species.strip()) + f'[Organism] AND ("20"[SLEN] : "22000"[SLEN]) AND biomol_genomic[PROP] NOT "unverified" {custom_query}'

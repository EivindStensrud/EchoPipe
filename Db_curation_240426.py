...
most_recent_database = os.path.abspath(args.old_database) # Input by the user! Does not exist on the first run, but is used on subsequent runs after curation and evaluation, where weird sequences should be removed.
...
def get_species_name(leaf):
    # Assuming the species identifier is the last two substrings of the leaf name.
    name_parts = leaf.name.split('_')
    return "_".join(name_parts[-3:-1])

def get_family_name(leaf):
    # Assuming the family identifier is the fourth last substring of the leaf name.
    name_parts = leaf.name.split('_')
    return name_parts[-5]
...
new_sequence_marked_list = add_symbol_to_new_sequence(new_potential_sequences) # Calls the function, which adds a * at the end of each header in the file. This modification is stored in the list new_sequence_marked.
...
new_sequence_marked_list = [line.replace('|', '_').replace(';', '_') for line in new_sequence_marked_list] # The list, now containing lines from most_recent_database and new_potential_sequences, have two characters swapped out in each of its lines.
...
# Create the MAFFT commandline object with "linsi" algorithm.
...
if duplicate_sequences:
    ...
    with open(f"Duplicate_entries.txt", "w") as file:
...
# Two files are created that are associated with the run. The user is meant to manually type in accession numbers that are weird and those that should be deleted from the database (after curation!)
with open(f"{run_name}_weird_sequences.txt", "w") as file:
    file.write("Write down accession numbers below - one per line - of those sequences with weird results based on manual curation.\n")

with open(f"{run_name}_sequences_to_delete.txt", "w") as file:
    file.write("Write down accession numbers below - one per line - of the weird sequences that remain dubious based on manual curation.\n")

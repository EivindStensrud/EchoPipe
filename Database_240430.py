...
parser = argparse.ArgumentParser(
    ...
    description="A tool to create, evaluate metabarcoding reference datebase.",
    epilog="Version 1.0")

parser.add_argument('input_file', type=str,
    help="The path to the input file containing the list of species.")
parser.add_argument('input_database', type=str,
    help="The path to the input reference database fasta file.")
parser.add_argument('-e', '--email', type=str, default=default_email, ### Make sure to remove the default from this and to make it a positional argument.
    help="The user's email address associated with the NCBI API key.")
parser.add_argument('-a', '--api_key', type=str, default=default_API_key, ### Make sure to remove the default from this and to make it a positional argument.
    help="The user's NCBI API key.")
parser.add_argument('-s', '--sort', action="store_true",
    help="The option allows for targeting the longest sequences.")
parser.add_argument('-c', '--maxcount', type=int, default=10000,
    help="The maximum number of accession numbers downloaded per species, default = 10 000.")
parser.add_argument('-l', '--maxlength', type=int, default=22000,
    help="The longest allowed sequence length for an accession number to be analysed, default = 22 000. This is optimized for the sake of mitochondrial sequence lengths. Do note other species may have longer mitochondrial sequences.")
parser.add_argument('-m', '--mitochondria', action='store_true',
    help="The search result will only return accession numbers that have been annotated as mitochondrial.")
parser.add_argument('-r', '--ribosomal', action='store_true',
...
parser.add_argument('-t', '--taxid', action='store_true', default="",
    help="The last saved taxid list is used. Only use if the same input from that run is wanted!")
parser.add_argument('-R', '--repeat', action='store_true', default="",
...
if not use_old_taxid: # Skips this step if the user decided to go with the previous taxid collection - saves time proportional to the number of input species.
    ...
    for idx, species in tqdm(enumerate(input_species_list, start=1), total=len(input_species_list), desc="Going through the species list"): # Loops through each species that originates from the users input.
        ...
        for _ in range(max_retries):
            try:
                species_timer = time.time() # Starts a timer that measures the time for each iteration.
                ...
                species_loop_duration = round(time.time() - species_timer, 2) # round(2) makes it round of the duration to two decimals.
...
min_length = 30 # Change this for different marker regions. Used as a cutoff for sequences we deem too short.
